...
export default function ChatBoard() {
  ...
  return (
    <main className="min-h-[calc(100vh-65px)] bg-[#FFF5F5] px-3 py-6 sm:px-4">
      <section className="mx-auto flex h-[calc(100vh-120px)] max-w-2xl flex-col overflow-hidden rounded-3xl border border-[#FECDD3] bg-[#FFF8F8] shadow-sm">
        {/* Header */}
        <header className="flex items-center gap-3 border-b border-[#FECDD3] bg-[#FFF8F8] px-4 py-3.5">
          <button
            type="button"
            onClick={() => navigate('/user')}
            className="flex h-9 w-9 items-center justify-center rounded-full text-[#5C574F] transition hover:bg-[#FECDD3]/60 hover:text-[#B91C1C]"
            aria-label="Back"
          >
            ←
          </button>
          ...
          <img
            ...
            alt={title}
            className="h-11 w-11 rounded-full object-cover ring-2 ring-[#FECDD3]"
          />

          <div className="min-w-0 flex-1">
            <h1 className="truncate font-serif text-[15px] font-medium text-[#1C1917]">
              {title}
            </h1>
            <p
              className={`text-xs ${
        ...
        {/* Messages */}
        <div className="flex-1 space-y-3 overflow-y-auto bg-[#FFF5F5] px-4 py-5">
          {messages.length === 0 && status === 'Connected' && (
            <div className="flex h-full flex-col items-center justify-center pt-10 text-center">
              <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-[#FECDD3]">
                ...
                <p
                  className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-[14px] leading-relaxed shadow-sm ${
                    mine
                      ...
                      : 'rounded-bl-md border border-[#FECDD3] bg-white text-[#1C1917]'
                  }`}
                >
                  {message.text}
                </p>
        ...
        {status === 'Connected' && (
          <form
            ...
            className="flex items-center gap-2 border-t border-[#FECDD3] bg-[#FFF8F8] px-3 py-3"
          >
            <input
              value={text}
              ...
              className="min-w-0 flex-1 rounded-full border border-[#FECDD3] bg-white px-4 py-2.5 text-sm text-[#1C1917] outline-none transition placeholder:text-[#A39E96] focus:border-[#B91C1C] focus:ring-2 focus:ring-[#B91C1C]/15"
            />
            <button
              type="submit"
              className="flex h-11 items-center justify-center rounded-full bg-[#B91C1C] px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#991B1B] active:scale-95"
            >
              Send
            </button>
          </form>
